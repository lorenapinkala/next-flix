...
function App() {
  ...
  //para que siempre muestre algo por defecto
  if(results.length){
    await fetchMovie(results[0].idf)
  }
  }
  ...
  return (
   <div>
    ...
    {/* buscador */}
    <form className='container mb-4' onSubmit={
      searchMovies
    }>
    ...
    {/* aca va todo el contedor del banner y del reproductor de video */}
    <div>
        <main>
          {movie ? (
            <div
    ...
    <div className='row'>
      {movies.map((movie)=>(
        <div key={movie.id} className='col-md-4 mb-3' onClick={()=>selectMovie(movie)}>
          <img src={URL_IMAGE+movie.poster_path} alt='imagen de la pelicula' height={600} width='100%'/>
          <h4 className='text-content'>{movie.title}</h4>
        </div>
      ))}
    </div>
